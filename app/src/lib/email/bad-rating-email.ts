...
export function badRatingHtml(args: BadRatingArgs): string {
  ...
  const noteLine = args.note?.trim()
    ? `<p style="margin:16px 0;padding:12px 16px;background:#FBF8F2;border-left:3px solid #D97878;font-style:italic;color:#34263F;">"${escapeHtml(args.note.trim())}"</p>`
  ...
  return `<!doctype html>
<html>
  <body style="margin:0;padding:0;background:#FBF8F2;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;color:#34263F;">
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#FBF8F2;padding:24px 0;">
      <tr><td align="center">
        <table width="560" cellpadding="0" cellspacing="0" style="background:#FFFFFF;border-radius:12px;overflow:hidden;border:1px solid #E2E8F0;">
          <tr><td style="padding:24px 28px;border-bottom:1px solid #E2E8F0;">
            ...
            <h1 style="margin:6px 0 0;font-size:20px;font-weight:500;color:#34263F;">${escapeHtml(args.tableLabel)} · ${args.rating}★</h1>
          </td></tr>
          <tr><td style="padding:20px 28px;">
            ...
            ${serverLine}
            <p style="margin:14px 0 0;color:#34263F;"><strong>Suggested action:</strong> ${escapeHtml(args.classification.suggestion)}</p>
          </td></tr>
          <tr><td style="padding:18px 28px 24px;border-top:1px solid #E2E8F0;background:#FBF8F2;">
            <a href="${escapeHtml(args.staffQueueUrl)}" style="display:inline-block;background:#34263F;color:#FFFFFF;text-decoration:none;padding:12px 20px;border-radius:8px;font-size:14px;font-weight:500;">Open live queue</a>
