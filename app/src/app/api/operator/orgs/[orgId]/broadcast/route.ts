  ...
  const html = `
    <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="font-family:-apple-system,BlinkMacSystemFont,sans-serif;line-height:1.5;color:#2B2539;background:#EBE9E4;">
      <tr><td style="padding:24px;">
        <p style="margin:0 0 4px;font-size:11px;letter-spacing:1.5px;text-transform:uppercase;color:#7B6767;">${escapeHtml(org.name)} · ${venues.length} venue${venues.length === 1 ? "" : "s"}</p>
        <h2 style="margin:0 0 16px;font-weight:500;">${escapeHtml(parsed.subject)}</h2>
        <div style="font-size:14px;color:#2B2539;white-space:pre-wrap;">${escapeHtml(parsed.body).replace(/\n/g, "<br>")}</div>
        <p style="margin:24px 0 0;font-size:12px;color:#7B6767;">
